...
public class ChessMatch {
	...
	private void validateTargetPosition(Position sourcePos, Position targetPos) {
		if (!board.getPiece(sourcePos).possibleMove(targetPos))
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == null) {
				Position capturePawnPosition;
				if (p.getColor() == Color.WHITE)
					capturePawnPosition = new Position(target.getRow() + 1, target.getColumn());
				else
					capturePawnPosition = new Position(target.getRow() - 1, target.getColumn());
				capturedPiece = board.removePiece(capturePawnPosition);
				capturedPieces.add(capturedPiece);
				piecesOnTheBoard.remove(capturedPiece);
			}
		}
